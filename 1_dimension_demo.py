...
while ballsStarted < numBalls:
    ...
    for a in range(0, ballsStarted):
        ...
        if (Ball.tooClose(potentialX, balls[a].posX, balls[a].radius + potentialRadius) or Ball.tooClose(potentialX, leftSide, balls[a].radius) or Ball.tooClose(potentialX, rightSide, balls[a].radius)):
            clear = False
...
for i in range(int(10 / sleepTime)):  # * This should be a 10 second animation.
    ...
    for i in range(numBalls): # TODO: Create better name for width
        ...
        if Ball.tooClose(leftSide, balls[i].posX, balls[i].radius) or Ball.tooClose(width, balls[i].posX, balls[i].radius):  # Doesn't seem to work great.
            balls[i].velX *= -1
    ...
    for ballA in range(numBalls):
        ...
        for ballB in range(ballA + 1, numBalls):
            ...
            if Ball.tooClose(balls[ballA].posX, balls[ballB].posX, balls[ballA].radius + balls[ballB].radius):

                newVelXA, newVelXB = Ball.oneDCollision(balls[ballA].mass, balls[ballA].velX, balls[ballB].mass, balls[ballB].velX)
